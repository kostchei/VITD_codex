...
- [ ] **R-04 - Grit, Flesh, injury, and healing** (p. 7). Replace generic health with Grit-first damage, Flesh injury, and settlement recovery. Pseudocode: `damage -> grit -> flesh -> injury; rest healsGrit; settlement healsFlesh`. Test overflow damage, injury creation, and healing restrictions.
...
- [ ] **R-10 - Regional/local map generation** (p. 10). Validate existing d6 terrain and local-density generation against the source tables. Pseudocode: `regionalDie -> terrain; densityDie -> count; localDie -> localTerrain`. Test every source-table range and generated-map invariant.
