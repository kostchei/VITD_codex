...
- [ ] **R-26 - Ruin room table** (pp. 22-27). Partial: source-faithful two-d6 room registry is implemented, preserving the printed duplicate `45` (Reliquary/Fountain) and absent `32`; detailed room-specific hazards, searches, saves, and effects remain to bind. Pseudocode: `roomChoices = registry[firstD6, secondD6]; resolveChosenRoom(room)`. Registry tests cover source endpoints and ambiguities.
...
- [ ] **R-33 - Trials of the Deep** (pp. 34-35). Partial: implemented all five trials described on these pages—Scale, Repetition, Change, Emptiness, and Sacrifice—with their state transitions, Simulacra, distances, rotations, rigging, and exit conditions. The sixth trial, Lies, is on the next page and remains to be combined with the escape procedure. Pseudocode: `trialState.advance(choice) -> exitOrConsequence`. Tests cover every available trial transition and exit condition.
